...
import { getDartProjectMarkerParserUserData } from "./dart_project_marker_parser";
...
export function enumConstantNaming(
  enumz: Enumz,
  enumConstant: EnumConstant
): string {
  let enumValueName: string = enumConstant.name;
  let trimPrefix = enumz.name;
  if (getDartProjectMarkerParserUserData(enumz)?.trimNamePrefixConfig) {
    trimPrefix =
      getDartProjectMarkerParserUserData(enumz)!.trimNamePrefixConfig!
        .trimPrefix;
  }

  let name = nameWithUnderscoresToCamelCase(
    enumValueName.replace(trimPrefix, ""),
    false
  );
  if (
    getDartProjectMarkerParserUserData(enumConstant)?.displayNameConfig
      ?.displayName
  ) {
    let displayNameConfig =
      getDartProjectMarkerParserUserData(enumConstant)?.displayNameConfig!;
    name = displayNameConfig.displayName!;
  }
    ...
    cxxFile.nodes.forEach((node) => {
      if (node.__TYPE == CXXTYPE.Clazz || node.__TYPE == CXXTYPE.Struct) {
        ...
        clazz.methods.forEach((method) => {
          ...
          if (
            getDartProjectMarkerParserUserData(method)?.displayNameConfig
              ?.displayName
          ) {
            let displayNameConfig =
              getDartProjectMarkerParserUserData(method)?.displayNameConfig!;
          ...
          setUserdata(method, userDataKey, {
            dartName: toDartStyleNaming(name),
          });
          ...
          method.parameters.forEach((param) => {
            ...
            //   let displayNameConfig =
            //     getDartProjectMarkerParserUserData(param)?.displayNameConfig!;
            //   paramName = displayNameConfig.displayName!;
            // }
        ...
        enumz.enum_constants.forEach((enumConstant) => {
          setUserdata(enumConstant, userDataKey, {
            dartName: enumConstantNaming(enumz, enumConstant),
          });
        });
