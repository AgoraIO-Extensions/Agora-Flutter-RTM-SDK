...
export default function BindingsRenderer(
    ...
    .map((cxxFile) => {
      let subContents = cxxFile.nodes
        ...
        .map((it) => {
          ...
          let clazzName = `${dartName(clazz)}NativeBinding`;
          ...
          let classExtendsBlock = shouldOverrideBaseClass
            ...
            : "";
          let constructorBlock = `${clazzName}(${
      ...
      // return {
      //   file_name: path.join(outputDir, `${dartFileName(cxxFile)}_impl.dart`),
  ...
  return [
    {
      file_name: path.join(outputDir, `rtm_native_bindings.dart`),
      file_content: content,
    },
  ];
  ...
  // return [
  //   ...renderResults,
  //   // callApiImplParamsJsonFile(parseResult, cxxFiles, outputDir),
  // ];
}
...
function callApiImplBlock(
  ...
  if (outVariable) {
    ...
    returnBlock = `if (result < 0) { throwExceptionHandler(code: result); }`;
  }

  let funcSignature = functionSignature(parseResult, method, {
    forceAddOverridePrefix: false,
  });
  let funcBlock = _trim(
    `
${apiType}
...
function callApiImplParamsJsonFile(
  ...
  return {
    file_name: path.join(outputDir, "call_api_impl_params_json.dart"),
    file_content: _trim(`
${defaultDartHeader}
